...
{
	std::unique_ptr<ArmorVector> resultVector(new ArmorVector);
	int resultSize = 0;
	int i = 0;

	while(resultSize != total_size && i < source.size())
	{
		if (source[i]->defense() <= max_defense && source[i]->defense() >= min_defense)
		{
			resultVector->push_back(source[i]);
			resultSize++;
		}
		i++;
	}
...
{
	//creating the 2d cache
	std::vector<std::vector<double>> cache;
	std::vector<double> defVec;
	std::vector<double> costVec;
	int n = armors.size();

	for(int i = 0; i < n; i++)
	{
		defVec.push_back(armors[i]->defense());
		costVec.push_back(armors[i]->cost());
	}
	...
	for(double i =  1; i <= n; i++)
	{
		for(int j = 1; j <= total_cost; j++)
		{
			if(costVec[i-1] <= j)
			{
				cache[i][j] =  std::max(cache[i-1][j], cache[i-1][j-costVec[i-1]] + defVec[i-1]);
			}
			else
			{
...
//getting result vector
std::unique_ptr<ArmorVector> resultVector( new ArmorVector);
int cost = total_cost;
double last = cache[n][total_cost];

for(int i = defVec.size(); (last > 0) && (i > 0); i--)
{
	if(last == cache[i-1][cost]){}
	else
	{
		cost -= costVec[i-1];
		last = cache[i-1][cost];
		resultVector->push_back(armors[i-1]);
	}
}
...
// Compute the optimal set of armor items with an exhaustive sea->cost(rch algorithm.
	...
	for(uint64_t bits = 0; bits < (pow(2,n)); bits++)
	{
		...
		for(int j = 0; j < n; j++)
		{
			...
			if((bits >> j) & 1)
			{
				...
				canCost += armors[j]->cost();
				canDef += armors[j]->defense();
			}
		}

			//best check
			if(canCost <= total_cost)
			{
				if((best == nullptr) || canDef > bestDef)
				{
